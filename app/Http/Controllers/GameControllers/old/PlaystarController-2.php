...
class PlaystarController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
		...
		$array = array('status_code' => 0, 'member_id' => $_REQUEST['access_token'], 'member_name' => $request['access_token'], 'balance' => $responsecurl['result']['balance']);
    ...
    {
		Log::emergency('Balance request: ' .  $request['access_token']);
		$token = PlaystarPlayers::where('sessionid', $_REQUEST['access_token'])->first();
		$explodeUserID = explode('-', $token->playerid);
		$currency = $explodeUserID[1];
		$playerId = $explodeUserID[0];
    ...
    public function createGame($playerId, $game_id, $casino_id)
    {
            ...
            if(!$localPlayerId) {
            $createPlayerId = PlaystarPlayers::create(['playerid' => $playerId, 'casino_id' => $casino_id, 'sessionid' => md5($playerId.'-'.now())]);
            }
