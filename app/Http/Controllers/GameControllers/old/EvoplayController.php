...
class EvoplayController extends Controller
{
    ...
    private $secret_key = '75352a7dad0ffa4f28c08ce2bd77f17d';
    private $version = '1';
        ...
        if($checkifExist) {
            ...
            $verifySign = md5($findoperator->apikey.'-'.$roundid.'-'.$findoperator->operator_secret);
                ...
                try {
                    ...
                    $OperatorTransactions = Gametransactions::create(['casinoid' => $findoperator->id, 'currency' => 'USD', 'player' => $playerId, 'ownedBy' => $findoperator->ownedBy, 'bet' => $roundingBet, 'win' => $roundingWin, 'gameid' => $gamedata, 'txid' => $transactionid, 'roundid' => $roundid, 'type' => 'slots', 'rawdata' => '[]']);
    ...
    public function win(Request $request)
    {
        ...
        $findoperator = Gameoptions::where('id', $getoperator)->first();
            ...
            if($rounding > 0) {
                $OperatorTransactions = Gametransactions::create(['casinoid' => $findoperator->id, 'currency' => 'USD', 'player' => $playerId, 'ownedBy' => $findoperator->ownedBy, 'bet' => '0', 'win' => $rounding, 'gameid' => $gamedata, 'txid' => $roundid, 'type' => 'slots', 'rawdata' => json_encode(['data' => $request->getContent()])]);
                if($rounding > 0) {
